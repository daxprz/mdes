...
- **Head tracking**: Aim direction from `global_position` (stable, no breathing feedback), skull anchored from `_spine[0]` (correct neck height). Aim blend increases to 100% during turns to override rest-pose snap. Distances are 2.5D projected.
- **Momentum/speed curves**: Asymmetric acceleration (`ACCEL_RATE=200`) and deceleration (`DECEL_RATE=600`). Monster builds speed slowly but brakes hard. Stride length scales with current speed (0.15 * speed, capped at 80% leg reach) — short choppy steps at low speed, long fluid strides at high speed. Step height also scales with speed ratio.
