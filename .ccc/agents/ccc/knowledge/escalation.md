# Escalation rubric — `ccc` (mdes) ↔ central `expert`

> When does the per-project `ccc` (mdes's Σ) handle
...
`/bug` and `/feature-request` auto-route through mdes's
...
- **Project-internal questions** ("which engineer owns module X?") —
  those go to mdes's own liaison or directly to the relevant
...
unambiguously, file a note in mdes's `effort/` directory
